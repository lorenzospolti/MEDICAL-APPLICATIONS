...
# -----------------------------------------------------------------------------
# STEP 1: FIX IMPUTATION (Task 1) - Add 'Suspension' to the list
# -----------------------------------------------------------------------------
imp_data <- baseline %>%
  dplyr::select(SUBJECT_ID, AGE, SEX, BMI, WEIGTH, FAMILIARITY, 
                DIAGNOSIS, ANTIBODY, GGCEF_T0, Suspension) # <--- ADDED Suspension HERE
...
# Prevent ID from being used as predictor
pred[, "SUBJECT_ID"] <- 0 
pred["SUBJECT_ID", ] <- 0

# Run Imputation
imputed_sets <- mice(imp_data, method = meth, predictorMatrix = pred, 
                     m = 5, maxit = 10, seed = 123, print = FALSE)

# Extract Completed Data (Now containing 'Suspension')
baseline_imputed <- mice::complete(imputed_sets, 1)

# Check if it exists now
colnames(baseline_imputed) # Should list "Suspension"
```
